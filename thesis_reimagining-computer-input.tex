...
\large Building a wearable Hand Controlled Input Device (\hyperref[orgf95a76e]{HCID}) for the advancing \hyperref[org88b0f70]{XR} revolution}
\hypersetup{
...
\item A semi-formal '\hyperref[org7b75b93]{5AA}' framework for analysing the capacities of BCI systems.

\item A new history of BCI, informed by our \hyperref[org7b75b93]{5AA} framework, through our lens of 'interface revolutions'.

\item A predictive futurology of BCI, specifically with regards to the imminent \hyperref[org88b0f70]{XR} revolution, opposing much of the prevailing wisdom.

\item The identification and analysis of the 'Achilles heel of \hyperref[org88b0f70]{XR}': that nobody is building high-throughput, high pervasiveness, ergonomic input for \hyperref[org88b0f70]{XR}.
...
\item Insights from our ongoing practical project to design and build a new Hand Controlled Input Device (\hyperref[orgf95a76e]{HCID}) to fix (6) and (8).
\end{enumerate}
\medskip
\end{mdframed}

\part{Theory \emph{or} The BCI Manifesto}
\label{sec:orge2668b0}
\chapter{Brains}
\label{sec:orgb747b4e}
\begin{mdframed}
\begin{description}
...
We (ab)use the term \emph{brain} in this way for no major reason other than to later satisfy the acronym \emph{\hyperref[org805b132]{Brain-Computer Interface} (BCI)} with a clearer definition.
\section{What makes humanity (brains) powerful}
\label{sec:org462dd0d}
...
\section{\hyperref[orgc53ce01]{General-Purpose Computing} (\hyperref[org5fbf4b0]{GPC})}
\label{sec:org1af8fa0}
Computers come in many sorts from simple sensors to supercomputers, but for humans, the assumed meaning of the term computer is the \emph{general-purpose (personal) computer (\hyperref[org5fbf4b0]{GPC})}.

\begin{mdframed}
\begin{description}
\item[{\label{orgc53ce01}General-Purpose Computing (\label{org5fbf4b0}GPC)}] Computing paradigm where the widest possible computing functionality is to provide some individual end users through a single unified interface.
\end{description}
\end{mdframed}

\textbf{\hyperref[org5fbf4b0]{GPC} has so far been the dominant interface paradigm defining how humans view themselves in relation to computers.}
...
\subsection{\hyperref[org86a798e]{Moore's blunder}}
\label{sec:org2068816}
...
\begin{mdframed}
\begin{description}
\item[{\label{org86a798e}Moore's Blunder}] Failing to recognise how the exponential progress of Moore's Law could soon make a currently smaller/slower/more expensive computing paradigm viable and competitive for functionality.
\end{description}
\end{mdframed}
...
Holding comment on human-human linguistics, we assert that it applies in its strongest form for human-computer interfaces, yielding \emph{\hyperref[orgfb49b5a]{Interface Relativity}}.

\begin{mdframed}
\begin{description}
\item[{\label{orgfb49b5a}Interface Relativity}] The structure of an interface (to a computer) affects its user's worldview or cognition, and thus people's perceptions and cognitive capabilities are relative to their computer interface.
\end{description}
\end{mdframed}
...
With \hyperref[orgfb49b5a]{interface relativity}, there is no denying that you see colours meaningfully differently when looking through thermal imaging goggles: you are literally perceiving parts of the electromagnetic spectrum that are undetectable otherwise.
...
\begin{mdframed}
\begin{description}
\item[{\label{org0a03d06}Congestion}] Reduced quality of service that occurs when a network node or link is carrying more data than it can handle.
\begin{itemize}
...
\begin{mdframed}
\begin{description}
\item[{\label{org97542da}System Uptime}] Proportion of time the communication channel is working and available. 
...
\begin{mdframed}
\begin{description}
\item[{Range\label{range} (\label{orgbfb9011}Frequency Band)}] The range of a spectrum over which (satisfactory) communication occurs. This concept allows a system to be decomposed into many discrete channels in which the bandwidth/noise/etc. may be treated differently.
\end{description}
\end{mdframed}
...
For this we suggest the new term \emph{Nervous System Interface (\hyperref[org559faaa]{NSI})}, to describe what's classically referred to as BCI, as a sub-category of our wider definition.

\section{Brains vs. computers vs. interfaces}
\label{sec:org9321a43}
...
\endlastfoot
\hline
\textbf{Primary Attributes} & Intelligence & Functionality \& efficiency & \hyperref[org7b75b93]{5AA}\\
\hline
\textbf{Theoretical Basis} & Cognitive sciences & Theory of computation & Information theory \& HCI\\
\hline
\textbf{Analog / Digital} & Analog & Digital & \hyperref[orge86f149]{ADC}\footnotemark\\
\hline
...
\\
\end{longtable}\footnotetext[29]{\label{org0708888}See \hyperref[sec:orgaba8fef]{Brain-computer ADC (Analog-to-Digital Conversion)} (\S \ref{sec:orgaba8fef}).}
}
}
...
\begin{mdframed}
\begin{description}
\item[{The Computational Theory of Mind\label{computational theory of mind} (\label{org579bdef}CTM)}] The brain is a computer and cognition/consciousness is a computation, bounded by the theory of computation.
\begin{itemize}
\item \emph{Note:} We regard the CMT as highly probable, though even a weaker case of the \hyperref[org579bdef]{CTM} (that \emph{much} of the brain's functioning is computational) suggests that the brain can be seamlessly interfaced with, and extended by, external computation.
\item \emph{Note:} The \emph{\hyperref[orged4c489]{Church-Turing-Deutsch Prinicple}}  implies the \hyperref[org579bdef]{CTM}.
\end{itemize}
\end{description}
\end{mdframed}
...
\begin{mdframed}
\begin{description}
\item[{The Extended Mind Thesis\label{extended mind thesis} (\label{org1c6a3dd}EMT)}] The mind extends beyond the brain\footnote{Brain in the classical sense of the organ in the skull.} and the body.
...
\begin{itemize}
\item \emph{Note:} We think the \hyperref[org579bdef]{CTM} strictly implies the \hyperref[org1c6a3dd]{EMT}. If the mind is computational, then like any other computer it should be possible to externalise subroutines (see also \emph{\hyperref[org7c2148f]{functional equivalence}}).
\item \emph{See also:} The \emph{\hyperref[org865f067]{Enhanceable Mind Thesis}}.
\end{itemize}
\end{description}
\end{mdframed}
...
\begin{mdframed}
\begin{description}
\item[{\label{org7c2148f}Functional Equivalence}] As a computation can be defined purely by its inputs and outputs; two systems that produce the same outputs given the same inputs are 'functionally' the same thing and should be treated as such.
\begin{itemize}
\item \emph{Lemma:} Applying \hyperref[org7c2148f]{functional equivalence} to the \hyperref[org579bdef]{CTM} yields two related concepts:
\begin{description}
...
\emph{Mental impedance} or \emph{Cognitive Load} is an abstract concept covering all the complex ways using an interface can take up more or less of your mental capacity, including; concentration, switching/learning costs, mental \hyperref[org0a03d06]{congestion}, \hyperref[noise]{noise}, sensory attenuation, etc.
...
Notice that our concept of accesibility has been derived from the information theory primitive of \emph{\hyperref[org97542da]{system uptime}}.
\subsection{Cost}
\label{sec:org0ed5789}
...
Note, \hyperref[org7b75b93]{5AA} \hyperref[mental impedance]{mental impedance} is high as humans do not naturally think and communicate in punched holes.
...
In \hyperref[org7b75b93]{5AA} terms, \hyperref[latency]{latency}, \hyperref[mental impedance]{mental impedance} and \hyperref[abstraction]{abstraction} are all so poor, that operation involves a second human.
...
The keyboard computing revolution was principally a revolution in \hyperref[org7b75b93]{5AA} 1) latency, 2) abstraction, 3) throughput and 4) mental impedance.

By the time electronic input for typewriter keyboards became viable, the typewriter was already over 200 years old and in large-scale use (\hyperref[org7b75b93]{5AA}-familiar).
...
The personal computing revolution was principally a revolution in \hyperref[org7b75b93]{5AA} 1) accessibility, both a) cost, and b) pervasiveness.
...
The GUI revolution was principally a revolution in \hyperref[org7b75b93]{5AA} 1) abstraction, particularly a) familiarity, and related, 2) mental impedance.
...
\section{\hyperref[org88b0f70]{XR} computing}
\label{sec:org06e75b4}
\begin{mdframed}
\begin{description}
\item[{\label{org88b0f70}XR (Extended Reality)}] Computing environments that are almost totally immersive\footnote{aka very high output bandwidth.} (VR) and/or pervasive (AR); tending to provide an overlay/alternative to 3D-spatial and sensory reality.
\end{description}
\end{mdframed}

At the time of writing we are at the onset of \emph{\hyperref[org88b0f70]{XR} computing revolution} (alt. \emph{\hyperref[wearable computing]{wearable computing}} revolution).
In the \hyperref[org88b0f70]{XR} revolution computers become i) \emph{totally immersive}-- capable of engaging the entire field of perception, and ii) \emph{almost fully pervasive}-- available for use in almost any situation.
In \hyperref[org7b75b93]{5AA} terms, this 1) overhauls throughput-- massively increasing bandwidth, whilst decreasing noise, 2) pushes pervasiveness near its limit-- becoming constantly 'wearable', and (3) considerably reduces impedance, both physical, and mental.
\textbf{We predict that \hyperref[org88b0f70]{XR} will combine the best qualities of desktop and mobile in a single interface and replace them both.}

Note, our focus is on \hyperref[org88b0f70]{XR} as a \hyperref[org5fbf4b0]{GPC} platform for maximally augmenting human intelligence.
This puts some distance between us and the in-vogue (narrower) outlook of \hyperref[org88b0f70]{XR} as a social-interactive entertainment metaverse of 3D virtual objects\footnote{We call this the 'Zuckervision'.}.
...
This unlocks many aspects of \hyperref[sec:org4fdd9eb]{Spatial computing} (\S \ref{sec:org4fdd9eb}) allowing computing configurations such as \emph{strolling desks}.

\section{Spatial computing}
\label{sec:org4fdd9eb}
\medskip
\begin{mdframed}
\begin{description}
...
Finally there is the more abstract issue of problems relating to \hyperref[sec:orgaba8fef]{brain-computer ADC} (\S \ref{sec:orgaba8fef}).
...
\section{Brain-computer \hyperref[orge86f149]{ADC} (Analog-to-Digital Conversion)}
\label{sec:orgaba8fef}
\medskip
\begin{mdframed}
\begin{description}
\item[{Brain-Computer \label{orge86f149}ADC (Analog-to-Digitial Conversion)}] That BCI interfaces must bridge the complex analog--digital divide between humans and computers.
\end{description}
\end{mdframed}
...
Evaluating through our \hyperref[org7b75b93]{5AA} framework, we identify significant \emph{physical obstruction}, increased \emph{latency}, and reduced \emph{throughput}, all in our opinion, unnecessarily.
...
\label{sec:org6283d51}
A list of \emph{aspiring} design goals for a new \hyperref[orgf95a76e]{HCID}, generated from our research so far.
\smallskip
...
\part{Conclusions}
\label{sec:org92b4d25}
\fi
\addcontentsline{toc}{part}{Conclusions}
\chapter*{Conclusions}
\label{sec:orgb8201bd}
...
We solved this by freezing the report to a certain prototype version (\hyperref[orgf95a76e]{HCID} v0.3)-- even though by submission our thinking had evolved and we had begun work on the next prototype.
...
Throughout this project, we have become increasingly convinced in our central thesis, that the \hyperref[org88b0f70]{XR} revolution is forthcoming, and a fast and pervasive input device will be required (we hope you have too).
...
\part{Appendices}
\label{sec:org9b1d80b}
\appendix
\chapter{Keymap Nomenclature}
\label{sec:orgd169eb1}
\section{General}
\label{sec:org5313c4e}
\begin{mdframed}
\begin{description}
...
\label{sec:orga3b1afa}
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../../../Images/artsey_keymap_grey.jpg}
\caption{\hyperref[orgf95a76e]{HCID} v0.3: Backside keymap \autocite{ARTSEYEasyFast}}
\end{figure}

\chapter{Design Decisions and Discussion}
\label{sec:org2652809}
The following is a (lengthy) retroactive record and discussion of the design decisions made up until this point (\hyperref[orgf95a76e]{HCID} v0.3).
