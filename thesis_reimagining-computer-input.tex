...
\item A semi-formal '\hyperref[orgafbfd0c]{5AA}' framework for analysing the capacities of BCI systems.

\item A new history of BCI, informed by our \hyperref[orgafbfd0c]{5AA} framework, through our lens of 'interface revolutions'.

\item A predictive futurology of BCI, specifically with regards to the imminent \hyperref[orgf7f8e78]{XR} revolution, opposing much of the prevailing wisdom.

\item The identification and analysis of the 'Achilles heel of \hyperref[orgf7f8e78]{XR}': that nobody is building high-throughput, high pervasiveness, ergonomic input for \hyperref[orgf7f8e78]{XR}.
...
\item Insights from our ongoing practical project to design and build a new Hand Controlled Input Device (\hyperref[org30e2275]{HCID}) to fix (6) and (8).
\end{enumerate}
\medskip
\end{mdframed}

\part{Theory \emph{or} The BCI Manifesto}
\label{sec:org381599c}
\chapter{Brains}
\label{sec:org91d74ae}
\begin{mdframed}
\begin{description}
...
We (ab)use the term \emph{brain} in this way for no major reason other than to later satisfy the acronym \emph{\hyperref[org89a0382]{Brain-Computer Interface} (BCI)} with a clearer definition.
\section{What makes humanity (brains) powerful}
\label{sec:org15e34ab}
...
\section{\hyperref[org628f737]{General-Purpose Computing} (\hyperref[orgb3382ea]{GPC})}
\label{sec:org6ffdb95}
Computers come in many sorts from simple sensors to supercomputers, but for humans, the assumed meaning of the term computer is the \emph{general-purpose (personal) computer (\hyperref[orgb3382ea]{GPC})}.

\begin{mdframed}
\begin{description}
\item[{\label{org628f737}General-Purpose Computing (\label{orgb3382ea}GPC)}] Computing paradigm where the widest possible computing functionality is to provide some individual end users through a single unified interface.
\end{description}
\end{mdframed}

\textbf{\hyperref[orgb3382ea]{GPC} has so far been the dominant interface paradigm defining how humans view themselves in relation to computers.}
...
\subsection{\hyperref[orga8601ec]{Moore's blunder}}
\label{sec:orgee50eac}
...
\begin{mdframed}
\begin{description}
\item[{\label{orga8601ec}Moore's Blunder}] Failing to recognise how the exponential progress of Moore's Law could soon make a currently smaller/slower/more expensive computing paradigm viable and competitive for functionality.
\end{description}
\end{mdframed}
...
Holding comment on human-human linguistics, we assert that it applies in its strongest form for human-computer interfaces, yielding \emph{\hyperref[org22c5c6a]{Interface Relativity}}.

\begin{mdframed}
\begin{description}
\item[{\label{org22c5c6a}Interface Relativity}] The structure of an interface (to a computer) affects its user's worldview or cognition, and thus people's perceptions and cognitive capabilities are relative to their computer interface.
\end{description}
\end{mdframed}
...
With \hyperref[org22c5c6a]{interface relativity}, there is no denying that you see colours meaningfully differently when looking through thermal imaging goggles: you are literally perceiving parts of the electromagnetic spectrum that are undetectable otherwise.
...
\begin{mdframed}
\begin{description}
\item[{\label{org1ee962c}Congestion}] Reduced quality of service that occurs when a network node or link is carrying more data than it can handle.
\begin{itemize}
...
\begin{mdframed}
\begin{description}
\item[{\label{org7eec694}System Uptime}] Proportion of time the communication channel is working and available. 
...
\begin{mdframed}
\begin{description}
\item[{Range\label{range} (\label{org9d3c606}Frequency Band)}] The range of a spectrum over which (satisfactory) communication occurs. This concept allows a system to be decomposed into many discrete channels in which the bandwidth/noise/etc. may be treated differently.
\end{description}
\end{mdframed}
...
For this we suggest the new term \emph{Nervous System Interface (\hyperref[org5662848]{NSI})}, to describe what's classically referred to as BCI, as a sub-category of our wider definition.

\section{Brains vs. computers vs. interfaces}
\label{sec:org70c7d2b}
...
\endlastfoot
\hline
\textbf{Primary Attributes} & Intelligence & Functionality \& efficiency & \hyperref[orgafbfd0c]{5AA}\\
\hline
\textbf{Theoretical Basis} & Cognitive sciences & Theory of computation & Information theory \& HCI\\
\hline
\textbf{Analog / Digital} & Analog & Digital & \hyperref[org633bfb2]{ADC}\footnotemark\\
\hline
...
\\
\end{longtable}\footnotetext[29]{\label{orgb2499c2}See \hyperref[sec:orgd30f58a]{Brain-computer ADC (Analog-to-Digital Conversion)} (\S \ref{sec:orgd30f58a}).}
}
}
...
\begin{mdframed}
\begin{description}
\item[{The Computational Theory of Mind\label{computational theory of mind} (\label{orgc0bac07}CTM)}] The brain is a computer and cognition/consciousness is a computation, bounded by the theory of computation.
\begin{itemize}
\item \emph{Note:} We regard the CMT as highly probable, though even a weaker case of the \hyperref[orgc0bac07]{CTM} (that \emph{much} of the brain's functioning is computational) suggests that the brain can be seamlessly interfaced with, and extended by, external computation.
\item \emph{Note:} The \emph{\hyperref[org221cb48]{Church-Turing-Deutsch Prinicple}}  implies the \hyperref[orgc0bac07]{CTM}.
\end{itemize}
\end{description}
\end{mdframed}
...
\begin{mdframed}
\begin{description}
\item[{The Extended Mind Thesis\label{extended mind thesis} (\label{org1e2274c}EMT)}] The mind extends beyond the brain\footnote{Brain in the classical sense of the organ in the skull.} and the body.
...
\begin{itemize}
\item \emph{Note:} We think the \hyperref[orgc0bac07]{CTM} strictly implies the \hyperref[org1e2274c]{EMT}. If the mind is computational, then like any other computer it should be possible to externalise subroutines (see also \emph{\hyperref[orgb2d01c7]{functional equivalence}}).
\item \emph{See also:} The \emph{\hyperref[orgc57c877]{Enhanceable Mind Thesis}}.
\end{itemize}
\end{description}
\end{mdframed}
...
\begin{mdframed}
\begin{description}
\item[{\label{orgb2d01c7}Functional Equivalence}] As a computation can be defined purely by its inputs and outputs; two systems that produce the same outputs given the same inputs are 'functionally' the same thing and should be treated as such.
\begin{itemize}
\item \emph{Lemma:} Applying \hyperref[orgb2d01c7]{functional equivalence} to the \hyperref[orgc0bac07]{CTM} yields two related concepts:
\begin{description}
...
\emph{Mental impedance} or \emph{Cognitive Load} is an abstract concept covering all the complex ways using an interface can take up more or less of your mental capacity, including; concentration, switching/learning costs, mental \hyperref[org1ee962c]{congestion}, \hyperref[noise]{noise}, sensory attenuation, etc.
...
Notice that our concept of accesibility has been derived from the information theory primitive of \emph{\hyperref[org7eec694]{system uptime}}.
\subsection{Cost}
\label{sec:org034d15a}
...
Note, \hyperref[orgafbfd0c]{5AA} \hyperref[mental impedance]{mental impedance} is high as humans do not naturally think and communicate in punched holes.
...
In \hyperref[orgafbfd0c]{5AA} terms, \hyperref[latency]{latency}, \hyperref[mental impedance]{mental impedance} and \hyperref[abstraction]{abstraction} are all so poor, that operation involves a second human.
...
The keyboard computing revolution was principally a revolution in \hyperref[orgafbfd0c]{5AA} 1) latency, 2) abstraction, 3) throughput and 4) mental impedance.

By the time electronic input for typewriter keyboards became viable, the typewriter was already over 200 years old and in large-scale use (\hyperref[orgafbfd0c]{5AA}-familiar).
...
The personal computing revolution was principally a revolution in \hyperref[orgafbfd0c]{5AA} 1) accessibility, both a) cost, and b) pervasiveness.
...
The GUI revolution was principally a revolution in \hyperref[orgafbfd0c]{5AA} 1) abstraction, particularly a) familiarity, and related, 2) mental impedance.
...
\section{\hyperref[orgf7f8e78]{XR} computing}
\label{sec:org456f5eb}
\begin{mdframed}
\begin{description}
\item[{\label{orgf7f8e78}XR (Extended Reality)}] Computing environments that are almost totally immersive\footnote{aka very high output bandwidth.} (VR) and/or pervasive (AR); tending to provide an overlay/alternative to 3D-spatial and sensory reality.
\end{description}
\end{mdframed}

At the time of writing we are at the onset of \emph{\hyperref[orgf7f8e78]{XR} computing revolution} (alt. \emph{\hyperref[wearable computing]{wearable computing}} revolution).
In the \hyperref[orgf7f8e78]{XR} revolution computers become i) \emph{totally immersive}-- capable of engaging the entire field of perception, and ii) \emph{almost fully pervasive}-- available for use in almost any situation.
In \hyperref[orgafbfd0c]{5AA} terms, this 1) overhauls throughput-- massively increasing bandwidth, whilst decreasing noise, 2) pushes pervasiveness near its limit-- becoming constantly 'wearable', and (3) considerably reduces impedance, both physical, and mental.
\textbf{We predict that \hyperref[orgf7f8e78]{XR} will combine the best qualities of desktop and mobile in a single interface and replace them both.}

Note, our focus is on \hyperref[orgf7f8e78]{XR} as a \hyperref[orgb3382ea]{GPC} platform for maximally augmenting human intelligence.
This puts some distance between us and the in-vogue (narrower) outlook of \hyperref[orgf7f8e78]{XR} as a social-interactive entertainment metaverse of 3D virtual objects\footnote{We call this the 'Zuckervision'.}.
...
This unlocks many aspects of \hyperref[sec:org5dcb1c3]{Spatial computing} (\S \ref{sec:org5dcb1c3}) allowing computing configurations such as \emph{strolling desks}.

\section{Spatial computing}
\label{sec:org5dcb1c3}
\medskip
\begin{mdframed}
\begin{description}
...
Finally there is the more abstract issue of problems relating to \hyperref[sec:orgd30f58a]{brain-computer ADC} (\S \ref{sec:orgd30f58a}).
...
\section{Brain-computer \hyperref[org633bfb2]{ADC} (Analog-to-Digital Conversion)}
\label{sec:orgd30f58a}
\medskip
\begin{mdframed}
\begin{description}
\item[{Brain-Computer \label{org633bfb2}ADC (Analog-to-Digitial Conversion)}] That BCI interfaces must bridge the complex analog--digital divide between humans and computers.
\end{description}
\end{mdframed}
...
Evaluating through our \hyperref[orgafbfd0c]{5AA} framework, we identify significant \emph{physical obstruction}, increased \emph{latency}, and reduced \emph{throughput}, all in our opinion, unnecessarily.
...
\label{sec:org09dec1d}
A list of \emph{aspiring} design goals for a new \hyperref[org30e2275]{HCID}, generated from our research so far.
\smallskip
...
\part{Conclusions}
\label{sec:orga47b1b4}
\fi
\addcontentsline{toc}{part}{Conclusions}
\chapter*{Conclusions}
\label{sec:orgafa5763}
...
We solved this by freezing the report to a certain prototype version (\hyperref[org30e2275]{HCID} v0.3)-- even though by submission our thinking had evolved and we had begun work on the next prototype.
...
Throughout this project, we have become increasingly convinced in our central thesis, that the \hyperref[orgf7f8e78]{XR} revolution is forthcoming, and a fast and pervasive input device will be required (we hope you have too).
...
\part{Appendices}
\label{sec:orgf7bf88a}
\appendix
\chapter{Keymap Nomenclature}
\label{sec:org96600ac}
\section{General}
\label{sec:org960d6c4}
\begin{mdframed}
\begin{description}
...
\label{sec:orgad216fc}
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../../../Images/artsey_keymap_grey.jpg}
\caption{\hyperref[org30e2275]{HCID} v0.3: Backside keymap \autocite{ARTSEYEasyFast}}
\end{figure}

\chapter{Design Decisions and Discussion}
\label{sec:org37c8e45}
The following is a (lengthy) retroactive record and discussion of the design decisions made up until this point (\hyperref[org30e2275]{HCID} v0.3).
